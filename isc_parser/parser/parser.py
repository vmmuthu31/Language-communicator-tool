...
MODEL_DIR = '%s/../dep-models' % os.path.dirname(os.path.abspath(__file__))
...
class Parser(ArcEager):
    ...
    def __init__(self, beamwidth=1, lang='hin', ud=False):
        ...
        self.template = Template(lang=lang, ud=ud)
        ud = '_ud' if (ud and lang == 'hin') else ''
        with open('%s/%s_parse%s.pkl' % (MODEL_DIR, lang, ud), 'rb') as fp:
            if PV >= 3:
